...
class GameSession:
    ...
    def update(
        ...
        *,
        ...
        """Advance gameplay, consuming every completed camera pose in order.

        Rendering may sample only the latest pose, but scoring must not lose real
        inference results merely because the render loop is slower than the pose
        worker. Camera callbacks publish completed bodies into a small
        chronological queue which is drained here on each game-loop update.
        """
        ...
        if not self.running:
            # Readiness only needs the newest body; never carry pre-start camera
            # history across chart time zero as scoring evidence.
        ...
        if not samples:
            # Preserve non-input bookkeeping (failure/outro clocks, etc.) while
            # MotionTracker and the scoring watermark deduplicate this repeated
            # capture timestamp.
            self._update_one(
                body,
                ready_to_start=ready_to_start,
