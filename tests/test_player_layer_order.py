...
from vaporstep.tracking_overlay import draw_lower_body_tracking_overlay
...
def test_calibration_character_is_finalized_after_external_overlays(monkeypatch) -> None:
    pygame.font.init()
    renderer = Renderer(pygame.Surface((640, 360)))
    calls: list[str] = []

    monkeypatch.setattr(renderer, "_draw_background", lambda *args, **kwargs: None)
    monkeypatch.setattr(renderer, "_draw_playfields", lambda *args, **kwargs: None)
    monkeypatch.setattr(renderer, "_draw_chains", lambda *args, **kwargs: None)
    monkeypatch.setattr(renderer, "_draw_notes", lambda *args, **kwargs: None)
    ...
    assert "character" not in calls
    calls.append("calibration-overlay")
    calls.append("tracking-overlay")
    draw_lower_body_tracking_overlay(renderer, BodyState())
    assert calls[-3:] == ["calibration-overlay", "tracking-overlay", "character"]
...
def test_character_layer_draws_without_global_alpha(monkeypatch) -> None:
    ...
    renderer._deferred_pose_figure = _pose()
    renderer.draw_deferred_character()
